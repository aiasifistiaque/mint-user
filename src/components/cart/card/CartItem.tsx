...
const CartItem = ({ image, id, name, price, qty }: CartItemProps) => {
  ...
  return (
    <Flex w="full" gap={4}>
      ...
      <Column>
        <Heading size="sm" color={colors?.primaryText}>
          {name}
        </Heading>
        ...
        <Box>
          <ButtonGroup bg="#efefef" borderRadius={4}>
            <Button onClick={handleRemoveOne} variant="ghost" size="md">
              -
            </Button>
            <Button variant="ghost" size="md">
              {qty}
            </Button>
            <Button onClick={handleAdd} variant="ghost" size="md">
              +
            </Button>
          </ButtonGroup>
        </Box>
