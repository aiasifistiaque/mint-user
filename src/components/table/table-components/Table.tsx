...
const TableCustom: FC<TableProps> = ({ table }) => {
	...
	useEffect(() => {
		const defaultFields = table?.data
			? table?.data?.filter((item: any) => item.type !== 'menu').map((item: any) => item.dataKey)
			: [];
	...
	useEffect(() => {
		const defaultPreferences = table?.data
			? table?.data?.filter((item: any) => item.default).map((item: any) => item.dataKey)
			: [];
